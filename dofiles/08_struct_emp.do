...
forvalues a = 1/`lim' {
	...
	* base year 
	sum pondera if sample == 1 & emplyd == 1 & sectorg == `a' 
	mat sector[`a',1]   = r(sum)

	sum pondera if sample == 1 & emplyd == 1 & sectorg == `a' & public == 0 
	mat sect_prv[`a',1]   = r(sum)
	
	sum pondera if sample == 1 & emplyd == 1 & sectorg == `a' & public == 1
...
forvalues a = 1/`lim' {
	...
	sum aux [w = pondera] if lf_samp == 1
	scalar v0 = r(mean)
...
if $m == 1 {
	...
	* Creates the structure sectoral matrix - targets
	...
	forvalues a = 1/`lim' {
		sum informal [w = pondera] if lf_samp == 1 & emplyd == 1 & sect_main == `a' 
...
forvalues a = 1/`lim_inf' {
	...
	sum aux [w = pondera] if lf_samp == 1 & sect_main == `a'
	scalar v0 = r(mean)
