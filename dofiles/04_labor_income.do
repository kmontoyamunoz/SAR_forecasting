...
* x vars 
loc ols_rhs           c.edad##c.edad urbano ib0.male##ib0.jefe
loc ols_rhs `ols_rhs' ib0.male#ibn.educ_lev salaried public_job informal
...
foreach sector of numlist `numb_sectors' {
	...
	foreach skill of numlist `numb_skills' {
		...
		*if "$use_saved_parameters" == "no" {
			regress `depvar' `ols_rhs' [pw = pondera] if sect_main == `sector' & skill == `skill'   & sample == 1
