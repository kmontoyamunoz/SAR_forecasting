...
* 			   Aug 30, 2023 - Corrected hhd with jefe
*===========================================================================
...
**************************************************
loc nl_incomes "remesas pensions capital"
...
forvalues i = 1/`lim' {
	...
	* calculates the growth rate of capital according to population growth 
	sum h_`x' [aw = pondera  ] if h_`x' > 0 & h_`x' <.
...
mtab1 region urban [aw = fexp_s] if jefe == 1 & h_remesas > 0 & h_remesas < ., sum(h_remesas) matrix(RT) med(MED) quanty(REM) total
mtab1 region urban [aw = fexp_s] if jefe == 1, sum(h_remesas) matrix(tp) quanty(TH)
...
forvalues i = 1(1)`r' {
	...
	* Calculates the growth rate of remittances from abroad according to population growth
	sum h_remesas [w = pondera] if h_remesas > 0 & h_remesas < .
	sca var0 = r(sum)
...
forvalues i = 1(1)`r' {
	...
	if `i' == 1 {
		...
		* Calculates the growth rate of remittances from abroad according to population growth
		sum h_remesas [aw = pondera]  if h_remesas > 0 & h_remesas <.
		sca var0 = r(sum)
		...
		if scalar(Tr) > 0 {
			...
			** Rural households by region	
			forvalues x = 1/`e' {
				gsort -_Iregion_`x' urban jefe id
				capture drop aux1
				gen double aux1 = sum(fexp_s)/N_r[`x',1] if jefe == 1
			...
			** Urban households by region	
			forvalues x = 1/`e' {
				gsort -_Iregion_`x' -urban jefe id
				capture drop aux1
				gen double aux1 = sum(fexp_s)/N_u[`x',1] if jefe == 1
	...
	if `i' >= 2 {
		...
		* Calculates the growth rate of remittances from abroad according to population growth & previous remittances growth
		...
		if scalar(Tr) > 0 {
			...
			** Rural households by region	
			forvalues x = 1/`e' {
				gsort -_I`region'_`x' urban jefe hid
				capture drop aux1
				gen double aux1 = sum(fexp_s)/N_r[`x',1] if jefe == 1
				replace aux2 = tr_r[`x',1] if aux1 <= SH_r[`x',1] & aux2 == .
			}
