...
gen     age_groups =  .
replace age_groups =  1 if edad >=  0 & edad <=  14
replace age_groups =  2 if edad >= 15 & edad <=  29
replace age_groups =  3 if edad >= 30 & edad <=  44
replace age_groups =  4 if edad >= 45 & edad <=  64
replace age_groups =  5 if edad >= 65 
...
if $weights == 0 {
	...
	* Check
	sum fexp_s [w = fexp_s]
	if abs(r(sum_w) - growth_pop_wdi[1,1]) > 500000 di in red "WARNING: Population differs in more than a million people with WDI."
